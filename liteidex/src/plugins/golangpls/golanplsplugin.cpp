...
bool GolanPlsPlugin::load(LiteApi::IApplication *app)
{
    ...
    if (manager) {
        auto factory = new GolangPlsHighlighterFactory(this);
        manager->addFactory(factory);
        m_pls->setHighlightFactory(factory);
        connect(app->editorManager(), &LiteApi::IEditorManager::editorAboutToClose, factory, &GolangPlsHighlighterFactory::removeHighlighterForEditor);
    }
